...
from tests.utils import create_dog, create_user
...
dog_same_id = {"Chispas": {"id": 1, "owner_email": user_standard["email"]}}

dog_same_name = {"Estrella": {"id": 2, "owner_email": user_standard["email"]}}


some_dogs = {
    "Estrella": {"id": 1, "owner_email": user_standard["email"]},
    "Chispas": {"id": 2, "owner_email": user_standard["email"]},
    "Rayo": {"id": 3},
}
...
def test_get_dogs_full(test_app: TestClient, some_dogs: dict):
    url = endpoint
...
def test_is_adopted(test_app: TestClient, some_dogs: dict):
    ...
    for name, data in some_dogs.items():
        ...
        assert (
            response_dog.status_code >= 200 and response_dog.status_code < 300
        ), response_dog.text
...
def test_adopt(test_app: TestClient):
    ...
    assert response_dog.json()["in_charge_id"] == user_standard["id"]
    assert response_dog.json()["is_adopted"] is False
    response_adopt = test_app.put(
        f"{url}?owner_email={user_standard['email']}", headers=headers
    )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            dog_same_id, marks=[pytest.mark.dependency(name="create_same_id_dog")]
        ),
        pytest.param(
            dog_same_name, marks=[pytest.mark.dependency(name="create_same_name_dog")]
        ),
    ],
)
def test_create_duplicate_dog(test_app: TestClient, some_dogs: dict):
    url = endpoint
    data_dog = {"id": 1, "owner_email": user_standard["email"]}
    response_create = create_user(test_app, "/api/v1/users", user_standard)
    assert response_create.status_code == 200, response_create.text
    data_login = {
        "username": user_standard["email"],
    ...
    for name, data in some_dogs.items():
        ...
        assert (
            ...
            and response_duplicate.status_code < 500
        ), response_duplicate.text
        assert response_duplicate.json() == {
            "detail": "Duplicate key: There is a dog with this data"
        }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            updates[2], marks=[pytest.mark.dependency(name="update_name_photo")]
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            updates[0], marks=[pytest.mark.dependency(name="update_name_inexistent")]
        ),
        pytest.param(
            updates[1], marks=[pytest.mark.dependency(name="update_photo_inexistent")]
        ),
        pytest.param(
            updates[2],
            marks=[pytest.mark.dependency(name="update_name_photo_inexistent")],
        ),
    ],
)
