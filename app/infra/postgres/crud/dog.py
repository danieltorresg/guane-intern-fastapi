...
class CRUDDog(CRUDBase[Dog, CreateDog, UpdateDog]):
    async def create(self, *, obj_in: CreateDog) -> Union[dict, None]:
        ...
        if type(dog_data["owner_id"]) == int:
            if not await user_service.get_one_by_id(id=dog_data["owner_id"]):
                raise HTTPException(
                    status_code=409, detail="User not found: Invalid user id"
                )
    ...
    async def delete(self, *, name: str, current_user: User) -> Union[dict, None]:
        ...
        if dog_deleted:
            if (
                dog_deleted[0]["owner_id"] == current_user["id"]
                or dog_deleted[0]["in_charge_id"] == current_user["id"]
            ):
                await self.model.filter(name=name).first().delete()
            else:
                raise HTTPException(
                    status_code=401,
                    detail="This user is unauthorized to deled this dog",
                )
    ...
    async def update(
        ...
        *,
        ...
        if dog_in_db:
            if (
                current_user["id"] == dog_in_db[0]["owner_id"]
                or current_user["id"] == dog_in_db[0]["in_charge_id"]
            ):
